<?php
	...
	$labShareID=aesDecrypt($_GET["key"]);
	
	$query="
		select	*
		from	lab_share
		where	labShareID='$labShareID'
	";
	...
	if($creditAvailable>0){
		...
		$query="
			update	lab_share
			set		creditAvailable=creditAvailable-1,
					lastAccessIP='".$_SERVER[REMOTE_ADDR]."',
					lastAccessDateTime=CURRENT_TIMESTAMP
			where	labShareID='$labShareID'
		";
		$result=mysql_query($query);
	}else{
