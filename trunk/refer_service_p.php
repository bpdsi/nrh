				...
				if($x>0 and $x<=10){
					...
					if($anm[$x]=="Gender"){
						if(trim($row["Gender"])==2){
							$return_value["patientDetail"]["Name"]["Gender"]=iconv("tis-620","utf-8","ผู้หญิง");
						}else{
							$return_value["patientDetail"]["Name"]["Gender"]=iconv("tis-620","utf-8","ชาย");
						}

					}elseif($anm[$x]=="Race"){
						$return_value["patientDetail"]["Name"]["Race"]=iconv("tis-620","utf-8","ไทย");
					}elseif($anm[$x]=="Nationality"){
						$return_value["patientDetail"]["Name"]["Nationality"]=iconv("tis-620","utf-8","ไทย");
					}elseif($anm[$x]=="Religion"){
						$return_value["patientDetail"]["Name"]["Religion"]=iconv("tis-620","utf-8","พุทธ");
					}else{
